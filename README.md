...
### 2.10 Нейронная сеть (MLPRegressor)

Использован многослойный персептрон `MLPRegressor` из библиотеки scikit-learn со следующей архитектурой: два скрытых слоя размером 128 и 64 нейрона, функция активации – ReLU, оптимизатор – Adam. Применены ранняя остановка и регуляризация (L2, alpha=0.0001). Входные числовые признаки масштабированы.
...
Результат сопоставим с бустинговыми моделями и превосходит дерево решений, что говорит о способности нейронной сети аппроксимировать нелинейные закономерности данных.

**Кривая обучения** демонстрирует устойчивую сходимость функции потерь к минимуму, без признаков переобучения.

![MLP Loss Curve](data/processed/nn_training_history.png)

**Гистограмма весов** первого скрытого слоя показывает симметричное распределение с центром в нуле, что свидетельствует о стабильном процессе оптимизации и отсутствии аномально больших весов.

![Weight Histogram (Input → First Hidden Layer)](data/processed/nn_weight_histograms.png)
...
Лучшей моделью по коэффициенту детерминации R² признан **XGBoost** (R² = 0.8048). CatBoost и нейронная сеть показали близкие результаты, уступая XGBoost незначительно.

### 3.3 Граф вычислительного конвейера DVC

С помощью команды `python -m dvc dag --dot` получено описание графа в формате DOT. С использованием библиотек `graphviz` и `subprocess` граф визуализирован и сохранён в файл.

![Вычислительный граф DVC](data/processed/dag.png)

На графе отражены три последовательных этапа: `eda`, `model_training` и `finalize`, а также их зависимости, входные и выходные файлы. Структура демонстрирует, что этап обучения зависит от результатов EDA, а финальный отчёт — от обученных моделей, что обеспечивает полную воспроизводимость.
...
2. Данные разделены на три непересекающиеся выборки: обучающую, валидационную и тестовую. Для прогнозирования использованы пять моделей: линейная регрессия, дерево решений, CatBoost, XGBoost и нейронная сеть (MLPRegressor).
3. Метрики качества (MSE, MAE, R²) вычислены на тестовой выборке и сведены в таблицу. Наилучшие результаты продемонстрировали градиентные бустинги (CatBoost / XGBoost), способные улавливать нелинейные зависимости и взаимодействия признаков. Дерево решений также показало неплохой результат, но уступает ансамблевым методам. Линейная регрессия, как и ожидалось, имеет самую низкую точность из-за нелинейности данных. Нейронная сеть показала сравнимые с бустингами результаты, однако требует больше вычислительных ресурсов и настройки.
...
**Рекомендация:** для задачи прогнозирования медицинских расходов целесообразно использовать CatBoost или XGBoost как наиболее точные и хорошо интерпретируемые модели в рамках данной работы.
